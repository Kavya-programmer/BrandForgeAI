...
export function InfluencerPanel({ data }: InfluencerPanelProps) {
  ...
  const avatarGradient = avatarColors[(data.name.charCodeAt(0) || 0) % avatarColors.length];
  ...
  return (
    <motion.div variants={STAGGER.container} initial="hidden" animate="show" className="space-y-4">
        ...
        <div className="px-6 pb-5">
          ...
          <div className="flex items-end gap-4 -mt-10 mb-4">
            ...
            <div className="pb-1">
              ...
              <p className="text-sm text-primary font-medium">{data.handle}</p>
            </div>
            <div className="ml-auto pb-1 text-right">
              <p className="text-xl font-bold text-foreground">{data.audienceSize}</p>
          ...
          {data.bio && (
            <div className="bg-secondary/60 rounded-xl p-3">
              <div className="flex justify-between items-start gap-2">
                <p className="text-sm text-foreground/85 leading-relaxed flex-1 italic">&ldquo;{data.bio}&rdquo;</p>
                <CopyButton text={data.bio} className="shrink-0" />
              </div>
            </div>
          )}
        </div>
        ...
        {data.contentStyle && (
          <motion.div variants={STAGGER.item} className="glass rounded-2xl border border-border/60 p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <CopyButton text={data.contentStyle} />
            </div>
            <p className="text-sm text-foreground/85 leading-relaxed">{data.contentStyle}</p>
          </motion.div>
        )}
        ...
        {data.influencerTypes && data.influencerTypes.length > 0 && (
          <motion.div variants={STAGGER.item} className="glass rounded-2xl border border-border/60 p-5">
            ...
            <div className="space-y-2">
              {data.influencerTypes.map((type, i) => (
      ...
      {/* Brand Collab Angle */}
      {data.brandCollabAngle && (
        <motion.div variants={STAGGER.item} className="glass rounded-2xl border border-border/60 p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Star className="w-4 h-4 text-violet-400" />
              <span className="section-label">Brand Collaboration Angle</span>
            </div>
            <CopyButton text={data.brandCollabAngle} />
          </div>
          <p className="text-sm text-foreground/85 leading-relaxed">{data.brandCollabAngle}</p>
        </motion.div>
      )}

      {/* Collab Ideas */}
      {data.collaborationIdeas && data.collaborationIdeas.length > 0 && (
        <motion.div variants={STAGGER.item} className="glass rounded-2xl border border-border/60 overflow-hidden">
          ...
          <div className="divide-y divide-border/40">
            {data.collaborationIdeas.map((idea, i) => (
